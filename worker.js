...
// ⬇️ NEW: QueueScheduler no longer needed — handled automatically by upsertJobScheduler if using delayed/repeatable jobs.
...
function spawnSofficeConvert(inputPath, outDir, targetFormat, timeoutMs = JOB_TIMEOUT_MS) {
  ...
  return new Promise((resolve, reject) => {
    child.on("close", (code) => {
      clearTimeout(timer);
      if (killed) return reject(new Error("soffice timed out"));
      if (code !== 0)
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      await downloadFromS3(s3InputKey, inputLocal);
